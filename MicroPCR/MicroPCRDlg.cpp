...
BEGIN_MESSAGE_MAP(CMicroPCRDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_LOGSAVE, &CMicroPCRDlg::OnBnClickedButtonLogsave)
	ON_BN_CLICKED(IDC_BUTTON_DEVICE, &CMicroPCRDlg::OnBnClickedButtonDevice)
END_MESSAGE_MAP()
...
BOOL CMicroPCRDlg::OnDeviceChange(UINT nEventType, DWORD dwData)
{
	...
	if (devices != NULL) {
		...
		{
			if (devices->GetAt(i).Compare(curDevice) == 0)
				return TRUE;
		}
	}
	
	SetWindowText(L"MicroPCR");
	...
	*/
	...
	if( isRecording )
	{
		...
		if( m_recordingCount % 20 == 0 ){
			...
			FileManager::log(log, logpath, logname);
		}
	}
...
void CMicroPCRDlg::OnBnClickedButtonLogsave()
{
	char szFilter[] = "텍스트 문서 (*.txt)|*.txt|All Files (*.*)|*.*||";
	CFileDialog dlg(FALSE, L"txt", L"텍스트 문서", OFN_HIDEREADONLY, (CString) szFilter);
	CString path;
	CString name;
	...
	if (IDOK == dlg.DoModal())
	{
		...
		logpath = path.TrimRight(L"\\" + name);
		logname = name;
	}
}

void CMicroPCRDlg::OnBnClickedButtonDevice()
{
	DeviceManagerDlg dlg;
	...
	if (IDOK == dlg.DoModal())
	{
		...
		for (int i = 0; i < 11; i++)
			serial[i] = (char) dlg.target.GetAt(i);

		if (device->OpenDevice(LS4550EK_VID, LS4550EK_PID, serial, TRUE))
		{
			curDevice = dlg.target;

			SetWindowText(L"MicroPCR - " + curDevice);
