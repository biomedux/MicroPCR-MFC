...
*	Note	 :	None
******************************************************************/
CStringArray* CDeviceConnect::CheckDevice(void)
{
	CStringArray *devices = new CStringArray();
	int nEntries = GetDevices();

	if( nEntries )
	{
		for (int i = 0; i < nEntries; i++)
			devices->Add((CString) m_DeviceList[i].SerialNumber);
